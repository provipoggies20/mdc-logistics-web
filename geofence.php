...
if ($equipmentResultDevices) {
    while ($row = $equipmentResultDevices->fetch_assoc()) {
        $equipmentTypes[] = $row['equipment_type'];
    }
} else {
    error_log("Devices equipment types query failed: " . $conn->error);
}
...
if ($equipmentResultKomtrax) {
    ...
    error_log("Komtrax equipment types query failed: " . $conn->error);
}
...
<head>
    ...
    <title>Geofence Monitoring</title>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function fetchRedHighlightCounts() {
        ...
        fetch('fetch_red_highlight_counts.php')
            ...
            .catch(error => {
                ...
                updateRedHighlightCounts({ maintenance: 0, monitoring: 0, geofence: 0, pending_edits: 0 });
            });
    }
    ...
    eventSource.onerror = function() {
        ...
        eventSource.close();
        setInterval(fetchRedHighlightCounts, 15000); // Poll every 15 seconds
    };
    const displayFields = [
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const fieldsReadonlySystemBase = [
        ...
        'speed_limit', 'days_no_gps', 'source_table'
    ];
    ...
    function loadGeofenceData(page, filter, search) {
    const table = document.getElementById('geofenceTable');
    table.classList.add('refreshing');
    fetch(`fetch_geofence.php?page=${page}&filter=${encodeURIComponent(filter)}&search=${encodeURIComponent(search)}`)
        .then(response => {
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
        ...
        .then(data => {
            ...
            const existingRows = new Map(Array.from(tbody.children).map(row => [row.cells[0].textContent, row]));
            tbody.innerHTML = data.geofenceLogs.length > 0 ? '' : '<tr><td colspan="5" class="no-results">No results found</td></tr>';

            data.geofenceLogs.forEach(log => {
                const row = existingRows.get(log.target_name) || document.createElement('tr');
                row.onclick = () => fetchVehicleInfo(log.target_name);

                const friendlyAssignment = assignmentMap[log.assignment] || log.assignment || 'Not Available';
                const timestamp = log.timestamp === '0000-00-00 00:00:00' ? 'Not Available' : log.timestamp;

                row.innerHTML = `
                    <td>${log.target_name}</td>
                    <td>${log.equipment_type || 'N/A'}</td>
                    <td>${friendlyAssignment}</td>
                    <td>${log.status}</td>
                    <td>${timestamp}</td>
                `;

                row.className = log.status.toLowerCase().includes('inside') ? 'inside-geofence' : 'overdue';
        ...
        .catch(error => {
            ...
            document.getElementById('geofenceTableBody').innerHTML = '<tr><td colspan="5" class="no-results">Error fetching data. Check console for details.</td></tr>';
            table.classList.remove('refreshing');
        });
}
    ...
    fetch(`fetch_vehicle_info.php?target_name=${encodeURIComponent(targetName)}`)
        ...
        .then(data => {
            ...
            if (data.error) {
                ...
                const currentDate = new Date('2025-07-13T21:11:00-07:00'); // Current date: July 13, 2025, 9:11 PM PST
                let daysElapsed = 0;
                let daysLapses = 0;
    ...
    function enterEditMode(data, displayFields) {
        ...
        fetch(`check_pending_edits.php?id=${encodeURIComponent(data.id)}&table=${encodeURIComponent(data.source)}`)
            ...
            .then(result => {
                ...
                // Define fields to be readonly in form
...
<select name="${field}" id="${field}" ${isReadonly ? 'disabled' : 'required'}>
                ...
                function calculateContractAndLapses() {
                    ...
                    const now = new Date('2025-05-17T13:26:00-07:00');
                    let contractDays = 0;
                    let elapsedDays = 0;
                    let lapsedDays = 0;
                    ...
                    saveButton.addEventListener('click', () => {
                        ...
                        .then(result => {
                            if (result.success) {
                                messageContainer.innerHTML = '<div class="message success">' + result.message + '</div>';
                                setTimeout(() => {
                                    fetchVehicleInfo(data.target_name); // Refresh vehicle info
                                }, 2000);
                            } else {
                ...
                if (cancelButton) {
                    cancelButton.addEventListener('click', () => {
                        fetchVehicleInfo(data.target_name); // Return to view mode
                    });
                }
            })
            .catch(error => {
    ...
    if (equipmentFilter) {
        equipmentFilter.addEventListener('change', function() {
            ...
            currentPage = 1;
            loadGeofenceData(currentPage, filterType, searchQuery);
        });
    }
    ...
    // Geofence Count Handling
    const geofenceBadge = document.getElementById('geofenceBadge');

    function updateGeofenceCount(count) {
        geofenceBadge.textContent = count;
        geofenceBadge.style.display = count > 0 ? 'inline' : 'none';
    }

    function fetchGeofenceCount() {
        fetch('fetch_red_highlight_counts.php')
            .then(response => response.json())
            .then(data => {
                if (data.error) {
                ...
                updateGeofenceCount(data.geofence);
            })
            .catch(error => console.error('Error fetching geofence count:', error));
    }

    setInterval(fetchGeofenceCount, 15000);
    window.addEventListener('focus', fetchGeofenceCount);
    fetchGeofenceCount();

    // Pending Count Handling (Main Admin Only)
    const pendingCountBadge = document.getElementById('pendingCountBadge');
    if (pendingCountBadge) {
        function updatePendingCount(count) {
            pendingCountBadge.textContent = count;
            pendingCountBadge.style.display = count > 0 ? 'inline' : 'none';
        }

        function fetchPendingCount() {
            fetch('fetch_pending_count.php')
                .then(response => response.json())
...
<div class="main-content">
    <h2><center>Geofence Monitoring</center></h2>
    ...
    <div class="table-sub-container">
        <h2>Geofence Status Overview</h2>
        <div class="table-wrapper">
            <table class="vehicle-table" id="geofenceTable">
                <thead>
                    <tr>
                        <th>VEHICLE</th>
